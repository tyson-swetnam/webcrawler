...
- **Seeding published state**: every workflow runs `scripts/seed_from_website.sh` before the pipeline. CI checks out `main`, whose committed `docs/` is a stale snapshot from the old self-hosted runs, so the script restores `docs/data/*` **and the full `docs/archive/` page set** from the `website` branch. Without the archive seeding the regenerated index only lists the months committed to `main` — the failure that hid April–July 2026 while those pages were live.
...
8. **Website**: `HTMLReportGenerator` renders the static site to both `output/` and `docs/`; `_heal_archive_pages()` fills in day pages for any date that has stored articles but no page (the archive index is a scan of `docs/archive/*.html`, so a missing page = an invisible day); per-article search stubs are indexed with the `pagefind` CLI into `docs/pagefind/`
